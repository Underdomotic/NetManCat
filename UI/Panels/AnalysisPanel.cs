...
public sealed class AnalysisPanel : UserControl, IDisposable
{
    ...
    public Action<string>? OnAddConnToGraph;
    public Action<string>? OnRemoveConnFromGraph;
    public Action<string>? OnAddProcToGraph;
    public Action<string>? OnRemoveProcFromGraph;
                          ...
                          BandwidthTracker bwTracker)
    {
        ...
        // v1.2: Grafico
        var menuSepGraph       = new ToolStripSeparator();
        var menuAddGraph       = new ToolStripMenuItem("📊  Aggiungi al Grafico");
        var menuRemGraph       = new ToolStripMenuItem("📊  Rimuovi dal Grafico");
        ...
        menuAddGraph.Click     += (_, _) => { if (_rightClickedKey != null) OnAddConnToGraph?.Invoke(_rightClickedKey); };
        menuRemGraph.Click     += (_, _) => { if (_rightClickedKey != null) OnRemoveConnFromGraph?.Invoke(_rightClickedKey); };
        menuAddProcGraph.Click += (_, _) => { if (_rightClickedGroup?.Key != null) OnAddProcToGraph?.Invoke(_rightClickedGroup.Key); };
        menuRemProcGraph.Click += (_, _) => { if (_rightClickedGroup?.Key != null) OnRemoveProcFromGraph?.Invoke(_rightClickedGroup.Key); };
        _contextMenu.Items.AddRange(new ToolStripItem[]
        {
            ...
            menuCollapseAll,
            menuSepGraph,
            menuAddGraph, menuRemGraph,
            menuAddProcGraph, menuRemProcGraph
        });
    ...
    private void ProcessConnections(List<TcpConnection> connections)
    {
        try
        {
            ...
            // v1.1: aggiorna config TCP probe a caldo dalla configurazione corrente
            _latencyProbe.SetTcpProbeConfig(_config.Config.TcpProbe);
    ...
    private void OnContextMenuOpening(object? sender, System.ComponentModel.CancelEventArgs e)
    {
        ...
        _contextMenu.Items[4].Visible = true;       // Espandi tutti
        _contextMenu.Items[5].Visible = true;       // Riduci tutti
        // Grafico
        _contextMenu.Items[6].Visible  = isSingle || isGroup;  // SepGraph
        _contextMenu.Items[7].Visible  = isSingle;             // Aggiungi connessione
        _contextMenu.Items[8].Visible  = isSingle;             // Rimuovi connessione
        _contextMenu.Items[9].Visible  = isGroup;              // Aggiungi processo
        _contextMenu.Items[10].Visible = isGroup;              // Rimuovi processo
        if (isGroup)
        {
